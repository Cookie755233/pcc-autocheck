...
export async function searchTenders(query: string): Promise<Tender[]> {
  const response = await fetch(
    `https://pcc.g0v.ronny.tw/api/searchbytitle?query=${encodeURIComponent(query)}`
  )
  if (!response.ok) {
    throw new Error(`Failed to fetch tenders for "${query}"`)
  }
  const data = await response.json()

  // Fetch full details for each tender
  const tendersWithDetails = await Promise.all(
    (data.records || []).map(async (tender: Tender) => {
      try {
        const detailResponse = await fetch(
          `https://pcc.g0v.ronny.tw/api/tender?unit_id=${tender.unit_id}&job_number=${tender.job_number}`
        )
        if (!detailResponse.ok) return tender
        ...
        console.error(`Failed to fetch details for tender ${tender.job_number}:`, error)
        return tender
      }
    })
  )
...
function filterDetailsByType(type: string, detail: any) {
  ...
  if (type.includes('招標')) {
    return {
      ...
      "採購資料:預算金額": detail["已公告資料:預算金額"] || detail["採購資料:預算金額"],
      "採購資料:預計金額是否公開": detail["已公告資料:預算金額是否公開"] || detail["採購資料:預計金額是否公開"],
      "招標資料:招標方式": detail["已公告資料:招標方式"] || detail["招標資料:招標方式"],
      "招標資料:決標方式": detail["已公告資料:決標方式"] || detail["招標資料:決標方式"],
      "招標資料:是否訂有底價": detail["已公告資料:是否訂有底價"] || detail["招標資料:是否訂有底價"],
      "招標資料:價格是否納入評選": detail["已公告資料:價格是否納入評選"] || detail["招標資料:價格是否納入評選"],
      "招標資料:招標狀態": detail["已公告資料:招標狀態"] || detail["招標資料:招標狀態"],
      "招標資料:公告日": detail["已公告資料:公告日"] || detail["招標資料:公告日"],
      "領投開標:截止投標": detail["已公告資料:截止投標"] || detail["領投開標:截止投標"],
      "領投開標:開標時間": detail["已公告資料:開標時間"] || detail["領投開標:開標時間"],
      "領投開標:收受投標文件地點": detail["已公告資料:收受投標文件地點"] || detail["領投開標:收受投標文件地點"],
    }
  }
  ...
  if (type.includes('無法決標')) {
    return {
      ...
      "無法決標公告:是否沿用本案號及原招標方式續行招標": detail["無法決標公告:是否沿用本案號及原招標方式續行招標"],
          ...
          try {
            ...
            for (const record of tender.records || []) {
              try {
                ...
                console.error(`Error processing version for ${tenderId}, date ${record.date}:`, versionError);
...
export const tenderService = {
    ...
    try {
      ...
      const enrichedData = {
        ...
        detail: {
          ...
          "採購資料:預計金額是否公開": tenderData.detail?.["採購資料:預計金額是否公開"],
          ...
          "招標資料:價格是否納入評選": tenderData.detail?.["招標資料:價格是否納入評選"],
          ...
          "領投開標:收受投標文件地點": tenderData.detail?.["領投開標:收受投標文件地點"],
          
          // 決標資料
          "決標資料:總決標金額": tenderData.detail?.["決標資料:總決標金額"],
          "決標資料:總決標金額是否公開": tenderData.detail?.["決標資料:總決標金額是否公開"],
          "決標資料:底價金額": tenderData.detail?.["決標資料:底價金額"],
          "決標資料:決標日期": tenderData.detail?.["決標資料:決標日期"],
          
          // 無法決標資料
          "無法決標公告:無法決標的理由": tenderData.detail?.["無法決標公告:無法決標的理由"],
          "無法決標公告:標的分類": tenderData.detail?.["無法決標公告:標的分類"],
          "無法決標公告:招標方式": tenderData.detail?.["無法決標公告:招標方式"],
          "無法決標公告:是否複數決標": tenderData.detail?.["無法決標公告:是否複數決標"],
          "無法決標公告:是否沿用本案號及原招標方式續行招標": tenderData.detail?.["無法決標公告:是否沿用本案號及原招標方式續行招標"],
          "無法決標公告:原招標公告之刊登採購公報日期": tenderData.detail?.["無法決標公告:原招標公告之刊登採購公報日期"],
          "無法決標公告:無法決標公告日期": tenderData.detail?.["無法決標公告:無法決標公告日期"],

          // 得標廠商資訊
          ...Object.fromEntries(
            Object.entries(tenderData.detail || {})
              .filter(([key]) => key.startsWith('投標廠商:') || key.startsWith('決標品項:'))
          ),
      ...
      // console.log('Saved tender version with enriched data:', {
      //   tenderId,
  ...
  extractFailureData(detail: any) {
    ...
    const isFailedTender = detail.type === '無法決標公告' || 
                           detail.type2 === '無法決標' || 
    ...
    return {
      reason: detail['無法決標公告:無法決標的理由'] || detail['無法決標資料:無法決標的理由'],
      failureDate: detail['無法決標公告:無法決標公告日期'] || detail['無法決標資料:無法決標日期'],
      nextAction: detail['無法決標公告:是否沿用本案號及原招標方式續行招標'] || 
  ...
  extractAwardData(detail: any) {
    ...
    const isAwardTender = (detail.type && detail.type.includes('決標')) || 
    ...
    console.log('Extracting award data from:', {
      ...
      fields: Object.keys(detail).filter(k => k.includes('決標') || k.includes('投標'))
    });
    ...
    if (winner === 'unknown') {
      ...
      for (const key of itemKeys) {
        const matches = key.match(/決標品項:第(\d+)品項:得標廠商(\d+):得標廠商/);
        if (!matches) continue;
        
        const [_, itemIndex, vendorIndex] = matches;
        winner = detail[key];
        winningBid = detail[`決標品項:第${itemIndex}品項:得標廠商${vendorIndex}:決標金額`] || totalAmount;
        break;
      }
    }
        ...
        for (const tender of tenders) {
          try {
            ...
            for (const record of data.records) {
              ...
              if (!processedData) {
                console.error(`Failed to process tender data for ${tenderId}, version date ${record.date}`);
                continue;
              }
              ...
              try {
                ...
                if (!existingVersion) {
                  ...
                  console.log(`Created version: ${tenderId}, date: ${record.date}, type: ${processedData.type}`);
                } else {
                  console.log(`Version already exists: ${tenderId}, date: ${record.date}, type: ${processedData.type}`);
                }
              } catch (error) {
  ...
  async saveTenderWithDateFilter(
    ...
    const filteredVersions = tender.versions.filter(version => {
      const versionDate = Number(version.date)
      const isRecent = versionDate >= thresholdTimestamp
      console.log(`Filtering version ${version.id} date: ${new Date(versionDate).toISOString()} - Within range: ${isRecent}`)
      return isRecent
    })
    
    if (filteredVersions.length === 0) {
      console.log(`Tender ${tender.id} skipped - no versions within date range`)
      return null
    }
    
    console.log(`Saving tender ${tender.id} with ${filteredVersions.length} versions (filtered from ${tender.versions.length})`)
